...
const trendingMediaController = async (req, res) => {
    try {
        ...
        // fetching data  
        const data = await fetchData("https://api.themoviedb.org/3/trending/all/day?language=en-US&page=" + page + `&api_key=${process.env.TMDB_KEY}`);
...
const movieMediaController = async (req, res) => {
    try {
        ...
        const data = await fetchData("https://api.themoviedb.org/3/discover/movie?include_adult=true&include_video=false&language=en-US&sort_by=popularity.desc&certification_country=US&certification=R&page=" + page + `&api_key=${process.env.TMDB_KEY}`);
...
const tvMediaController = async (req, res) => {
    try {
        ...
        const data = await fetchData("https://api.themoviedb.org/3/discover/tv?include_adult=true&include_null_first_air_dates=false&language=en-US&sort_by=popularity.desc&page=" + page + `&api_key=${process.env.TMDB_KEY}`);
